from achaea.assist import assisted
...
spirituality_aliases = [
    ...
    ("^her$", "hunt heresy", lambda matches: eqbal("hunt heresy")),
    (
        "^seek(?: (.+))?$",
        "angel seek t/[]",
    ...
    ("^abeck$", "angel beckon all", lambda matches: eqbal("angel beckon")),
    (
        "^beck$",
        "angel beckon t",
        lambda matches: eqbal(f"angel beckon {matches[0] or '&tar'}"),
    ),
    ("^sear(?: (.+))?$", "sear t/icewalldir", lambda matches: sear(matches)),
    (
        "^judge(?: (.+))?$",
        "judge t/[]",
    ...
    (
        "^fly(?: (.+))?$",
        "angel lift t/[]",
        lambda matches: eqbal(f"angel lift {matches[0] or ''}"),
    ),
    (
        "^trace(?: (.+))?$",
        "angel trace t/[]",
